...
@login_required
def ajax_search_packages(request, uuid):
    """
        Search packages to follow
    """
    user = request.user
    profile = user.get_profile()

    if uuid != profile.uuid:
        return HttpResponse(json.dumps("Permission denied!"), mimetype='application/json', status=403)

    installed_pkg_pks = set(inst.package for inst in InstalledPackage.objects.filter(owner=user.pk).only('package').distinct())

    # we've got a POST request which means search
    if request.POST:
        search = request.POST.get('search', "")
        if (not search) or (len(search) < 3):
            return HttpResponse(json.dumps([]), mimetype='application/json')

        safe_search = search.replace(' ', "_").lower()
        is_search = True
        # case sensitive search, limit to 300 results
        query = Package.objects.filter(name__contains=search)[:300]
    else:
        is_search = False
        query = Package.objects.filter(pk__in=installed_pkg_pks)

    record = []
    # temporary data structure for easier indexing when checking
    # followed/installed verions. Can't use dict in the JS because of shared code
    # initialize here b/c record may be cached and not initialized in a loop
    # also used to bulk fetch versions below too
    record_dict = {}
    pkg_names_pks = {} # used for sorting below

    # fetch all Package objects matching our query
    for obj in query.only('name', 'type', 'website'):
        # for same name packages with different type
        if not pkg_names_pks.has_key(obj.name):
            pkg_names_pks[obj.name] = []
        pkg_names_pks[obj.name].append(obj.pk)

        pkg = {
                'pk' : obj.pk, # NB: this stays due to how data is iterated/displayed in JS
                'n' : obj.name,
                'i' : obj.index_url(),
                'w' : obj.website,
                'img' : obj.type_img_48_url(),
                't' : obj.get_type_display(),
                'f' : 0,
                'v' : {},  # version info
                'vsort' : [], # ordered version pks
            }
        # initialize some private data
        if is_search:
            pkg['f'] = int(pkg['pk'] in installed_pkg_pks)
        else:
            pkg['f'] = 1 # if not searching we always display followed packages

        record.append(pkg)
        record_dict[pkg['pk']] = pkg


    # fetch all possible PackageVersion objects for our Package set
    ver_query = PackageVersion.objects.filter(
                        package__in=set(pkg['pk'] for pkg in record)
                    ).only(
                        'package',
                        'version'
                    ).order_by('package', '-released_on').distinct()
    for ver in ver_query:
        versions = record_dict[ver.package_id]['v']

        # NB: using PK as index to avoid reading the version below when
        # calculating if version is in app or followed!
        versions[ver.pk] = {
            'v' : ver.version,
            'f' : 0,
        }
        ...
        # save sort order
        record_dict[ver.package_id]['vsort'].append(ver.pk)

    # used for version follow check filters below.
    # if not searching this is the same as installed_pkg_pks
    # when searching contains PKs of non followed packages too
    pkg_query_pks = set(pkg['pk'] for pkg in record)

    # only check if installed/followed for versions which are from this user
    # and match the set of packages from the query above
    inst_versions = InstalledPackage.objects.filter(
                        owner=user.pk,
                        package__in=pkg_query_pks,
                        version__gt=0
                    ).values('version', 'package').distinct().annotate(Sum('application'))

    # now check which PackageVersion objects are from apps and which are manually followed
    for inst in inst_versions:
        # this should never raise an IndexError b/c PKs are the same as in Package query
        pkg = record_dict[inst['package']]

        if inst['application__sum'] == 0: # not in app, manually followed
            pkg['v'][inst['version']]['f'] = 1
        else:  # imported from app
            pkg['v'][inst['version']]['f'] = -1

    # clear data structures and re-initialize
    # this is a hack but we need it due to JS constraints
    record = []

    # return sorted by name
    names = pkg_names_pks.keys()
    names.sort()
    for name in names:
        for pkg_pk in pkg_names_pks[name]:
            record.append(record_dict[pkg_pk])

    return HttpResponse(json.dumps(record), mimetype='application/json')
